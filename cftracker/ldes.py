...
def PSR(response,rate):
    ...
    r=(max_response-response)/(delta+1e-10)
    r[np.isnan(r)]=np.inf
    return np.min(r)
...
class LDES(BaseCF):
    ...
    def logupdate(self,init,img,pos,tmp_sc,tmp_rot):
        ...
        if self.is_rotation:
            ...
            patch=cv2.resize(patchO,self.window_sz0,interpolation=cv2.INTER_LINEAR)
        ...
        patchL=cv2.resize(patchL,self.scale_sz_window,cv2.INTER_LINEAR)
