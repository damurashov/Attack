...
class Pioneer:
	...
	def __receive_heartbeat(self, blocking=False, timeout=None):
		self.__mavlink_socket.wait_heartbeat(blocking, timeout)
		if self.__logger:
	...
	@property
	def quaternion(self):
		if self.__telemetry["ATTITUDE_QUATERNION"] is not None:
			return [self.__telemetry["ATTITUDE_QUATERNION"].q1,
				self.__telemetry["ATTITUDE_QUATERNION"].q2,
				self.__telemetry["ATTITUDE_QUATERNION"].q3,
				self.__telemetry["ATTITUDE_QUATERNION"].q4,]
		return None

	@property
	def euler_angles(self):
		if self.__telemetry["ATTITUDE"] is not None:
			return [self.__telemetry["ATTITUDE"].roll,
				self.__telemetry["ATTITUDE"].pitch,
				self.__telemetry["ATTITUDE"].yaw,]
		return None
	...
	def rc_channels(self, roll, pitch, yaw, throttle, mode):
		"""
		:param roll: [-1.0; 1.0]
		:param pitch: [-1.0; 1.0]
		:param yaw:  [-1.0; 1.0]
		...
		"""
		def normalize(rc):
			return int(rc * 500 + 1500)

		mode_convert = {0: int(1000), 1: int(1500), 2: int(2000)}
